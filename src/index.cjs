...
function setRootAttribute(svg, name, value) {
  assertSvg(svg);
...
function injectTitle(svg, title) {
  assertSvg(svg);
...
function minifySvg(svg, options = {}) {
  assertSvg(svg);
...
function setColor(svg, color, options = {}) {
  assertSvg(svg);
